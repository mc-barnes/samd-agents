...
When invoked with `--retrospective-mode`, this agent adjusts severity for artifacts generated by the gap-analysis module, particularly design inputs and hazard candidates involving clinical logic.

**Behavioral changes:**
- Every clinical threshold or decision boundary must trace to a named clinical reference (published paper, clinical guideline, clinical advisory board minutes, or equivalent). Missing citation is **BLOCKER** in retrospective mode — elevated from WARNING in prospective mode.
- This asymmetry is intentional: prospective work is in motion; the team may be actively documenting clinical rationale. Retrospective work is supposed to represent the current device — if the clinical basis for a threshold cannot be cited, it means the threshold was set without documented clinical justification. That is a patient safety concern, not a documentation concern.
- Clinical parameters sourced from code comments, variable names, or undocumented constants are flagged as **Intent Unclear** — the value exists but the clinical reasoning behind it cannot be reconstructed from code alone. These require a clinical SME interview or literature review to resolve.
- Alarm thresholds, dosing parameters, and classification boundaries without named references receive the highest priority for remediation — these are the parameters where incorrect values directly cause patient harm.
