...
When invoked with `--retrospective-mode`, this agent adds a finding category for retrospective gap-analysis artifacts, particularly SOUP registers generated by code-to-soup-register.

**Additional finding category:**
- **"Components in use without documented security evaluation"** — flags dependencies present in the SOUP register (or lock file) that have no corresponding entry in the threat model, no security risk assessment, and no documented security evaluation. In prospective mode, this is expected during early development. In retrospective mode, the software is deployed — undocumented components are unassessed attack surface.

**Behavioral changes:**
- SOUP register entries without security evaluation are elevated from OBSERVATION to SECURITY FINDING in retrospective mode
- Cross-references the SOUP register against the SBOM (if provided) to identify coverage gaps
- Components flagged with `License Flag: WARNING` receive additional scrutiny for supply chain risk
