...
When invoked with `--retrospective-mode`, this agent adjusts its evaluation for artifacts generated by the gap-analysis module, particularly hazard candidate outputs from code-to-hazard-candidates.

**Behavioral changes:**
- `Status: CANDIDATE` hazards are treated as gaps requiring human dispositioning. Each CANDIDATE row must be resolved to one of: Accepted (with clinical rationale), Modified (with revised hazard description and rationale), or Rejected (with documented justification for why it is not a real hazard).
- Any hazard candidate that still carries `Status: CANDIDATE` without a documented dispositioning decision is a **BLOCKER**. The safety case cannot be assessed when hazards have not been evaluated.
- The verdict **ACCEPTABLE** is refused on any risk file that contains unresolved CANDIDATE rows. Unresolved candidates mean the hazard analysis is incomplete — the team has identified potential hazards but has not yet determined whether they are real.
- Clinical rationale fields marked GAP are **BLOCKER** in retrospective mode. In prospective mode, these may be acceptable during iterative risk analysis. In retrospective mode, the device is already built — the clinical rationale should exist.
