...
When invoked with `--retrospective-mode`, this agent adjusts its evaluation for artifacts generated by the gap-analysis module (code-to-soup-register, code-to-design-inputs, code-to-hazard-candidates).

**Behavioral changes:**
- Empty rationale fields (marked GAP) are **BLOCKER** findings, even if all other fields are populated. In prospective mode, empty rationale may be acceptable during early drafting. In retrospective mode, the artifact is supposed to represent the current state of the device — empty rationale means the design decision is undocumented.
- The verdict **ACCEPTABLE** is refused on any retrospective artifact that contains unaddressed gaps. The lowest possible passing verdict is NEEDS REVISION with a clear remediation list.
- The companion gap report must be referenced in the artifact. If a gap report exists but is not cross-referenced, issue a WARNING for traceability.
- Gap report items must map to specific artifact rows. Orphan gap items (gaps that don't correspond to an artifact row) are a BLOCKER — they indicate the gap report and artifact have diverged.
